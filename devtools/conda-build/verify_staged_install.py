...
def verify_receipts(
    ...
    routes = list(evidence_dir.rglob("argdigest-conda-route.json"))
    ...
    _require(
        route.get("schema") == "argdigest.conda-route@1", "Unexpected route schema"
    )
    ...
    _require(
        subject.get("repository") == "uibcdf/argdigest", "Wrong producer repository"
    )
...
def verify_installed(
    ...
    _require(argdigest.__version__ == version, "Imported module version mismatch")
    _require(
        Path(argdigest.__file__).resolve().is_relative_to(prefix.resolve()),
        "Imported the source checkout instead of the installed package",
    )
    cli = (
        prefix / "Scripts" / "argdigest.exe"
        if sys.platform == "win32"
        else prefix / "bin" / "argdigest"
    )
