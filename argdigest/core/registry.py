...
class Registry:
    ...
    def run(cls, kind: str, rules: list[str | Any], value: Any, ctx: Any) -> Any:
        logger.debug(
            f"Starting pipelines for kind='{kind}' on argument='{ctx.argname}'"
        )
        ...
        for rule in rules or []:
            ...
            if isinstance(rule, str):
                ...
                if fn is None:
                    logger.debug(
                        f"Rule '{rule}' not found in kind='{kind}'. Available: {list(pipelines.keys())}"
                    )
                    continue
            ...
            if isinstance(rule, type) and hasattr(rule, "model_validate"):
                ...
                logger.debug(
                    f"Running Pydantic model '{rule.__name__}' on argument='{ctx.argname}'"
                )
                try:
                    start = time.perf_counter() if do_profile else 0
