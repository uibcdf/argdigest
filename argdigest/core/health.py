...
def run_health_check() -> dict[str, dict[str, Any]]:
    ...
    try:
        ctx = Context(
            function_name="health.check", argname="value", value="bad", all_args={}
        )
    ...
    try:
        previous_profile = getattr(
            getattr(smonitor, "_config", object()), "profile", "user"
        )
    except Exception:
        previous_profile = "user"
    ...
    try:
        ...
        msg_user = resolve(
            code=CATALOG["exceptions"]["DigestTypeError"]["code"], extra=extra
        )
        ...
        report["profiles"] = {
            "ok": msg_user != msg_dev,
            "detail": "user/dev differ"
            if msg_user != msg_dev
            else "user/dev identical",
        }
    except Exception as exc:
