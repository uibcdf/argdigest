...
def resolve_standardizer(
    ...
    if isinstance(standardizer, str):
        ...
        if not module_path or not attr:
            raise ValueError(
                "standardizer must be a callable or 'module.path:function'"
            )
...
def _merge_digesters(
    target: dict[str, Callable[..., Any]], incoming: dict[str, Callable[..., Any]]
) -> None:
...
def _collect_digesters_from_module(
    module: ModuleType, output: dict[str, Callable[..., Any]]
) -> None:
...
def load_argument_digesters(
    ...
    if style != "auto":
        raise ValueError(
            "digestion_style must be 'auto', 'registry', 'package', or 'decorator'"
        )
