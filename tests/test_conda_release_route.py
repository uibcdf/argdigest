...
SPEC = importlib.util.spec_from_file_location("argdigest_release_route", SCRIPT)
...
def test_required_ci_must_have_exact_identity_and_success(monkeypatch, wrong_field):
    ...
    with pytest.raises(route.ReleaseRouteError, match="no successful exact-commit run"):
        route.verified_workflow_runs(
            "uibcdf/argdigest", SHA, [route.FULL_MATRIX], "token"
        )
...
@pytest.mark.parametrize("distributions", [[{"basename": "noarch/argdigest-py_0"}], []])
...
def test_public_poststate_matches_the_exact_built_file(monkeypatch, tmp_path):
    ...
    package = tmp_path / "argdigest-0.11.0-py_0.tar.bz2"
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: subprocess.CompletedProcess(
            args=[], returncode=0, stdout=json.dumps({"argdigest": [record]}), stderr=""
        ),
    )
